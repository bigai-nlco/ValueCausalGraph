...
with torch.no_grad():
    ...
    for name, char in players.items():
        ...
        for value_name, group in grouped:
            ...
            for qbn in range(question_batch_no):
                groupagreement = groupagreementall[
                    qbn * MAX_QUESTIONS_PER_BATCH : (qbn + 1) * MAX_QUESTIONS_PER_BATCH
                ]
                ...
                for groupmember in zip(groupagreement, groupquestion, groupitem):
                    ...
                    if SAMPLE_MODE == "PROMPT_SAE_INTERV":
                        prompt, role_setting = generate_question_norole(
                            a, q, qi, ALLOW_UNSURE_ANSWER
                        )
                    elif SAMPLE_MODE == "ROLES":
                        prompt, role_setting = generate_question(
                            name, trait, a, q, qi, ALLOW_UNSURE_ANSWER
                        )
                    else:
                ...
                if SAMPLE_MODE == "ROLES":
                    # while True:
                    if sae:
                        questions_tokens = model.tokenizer(
                            questions,
                            return_tensors="pt",
                            padding=True,
                            truncation=True,
                        ...
                        gen_tokens = model.generate(
                            questions_tokens.input_ids, max_new_tokens=20, verbose=False
                        )
                        ...
                        questions_padded = model.tokenizer.batch_decode(
                            questions_tokens.input_ids
                        )
                        for question, gen_text in zip(questions_padded, gen_texts):
                            gen_answer = judge_answer(
                                gen_text.lower()[len(question) :].strip()
                            )
                        ...
                        gen_answers = [
                            gen_text[0]["generated_text"][len(question) :].lower()
                            for question, gen_text in zip(questions, gen_texts)
                        ]
        ...
        def get_cached_feature_of_value(prompt, token_id):
            ...
            topk_values, topk_indice = torch.topk(
                feature_of_value, int(0.005 * len(feature_of_value))
            )
...
with torch.no_grad():
    ...
    for value_name, group in grouped:
        ...
        for trial in range(10):
            ...
            modified_answer = (
                model.tokenizer.batch_decode([torch.argmax(cmc[0][-1])])[0]
                .strip()
                .lower()
            )
            original_answer = (
                model.tokenizer.batch_decode([torch.argmax(logits[0][-1])])[0]
                .strip()
                .lower()
            )
