...
if DATA_SPLIT == "30clearori":
    df_valuebench = pd.read_csv(
        os.path.join(
            LOCAL_SAE_MODEL_PATH, "value_data/value_orientation_30clearori.csv"
        )
    )
elif DATA_SPLIT == "other":
    df_valuebench = pd.read_csv(
        os.path.join(LOCAL_SAE_MODEL_PATH, "value_data/value_orientation_other.csv")
    )
elif DATA_SPLIT == "valuebenchtrain":
    df_valuebench = pd.read_csv(
        os.path.join(LOCAL_SAE_MODEL_PATH, "value_data/value_orientation_train.csv")
    )
elif DATA_SPLIT == "valuebenchtest":
    df_valuebench = pd.read_csv(
        os.path.join(LOCAL_SAE_MODEL_PATH, "value_data/value_orientation_test.csv")
    )
elif DATA_SPLIT == "valuebenchteaser":
    df_valuebench = pd.read_csv(
        os.path.join(LOCAL_SAE_MODEL_PATH, "value_data/value_orientation_teaser.csv")
    )
elif DATA_SPLIT == "valuebenchall":
    df_valuebench = pd.read_csv(
        os.path.join(LOCAL_SAE_MODEL_PATH, "value_data/value_orientation.csv")
    )
else:
...
if DATA_SPLIT_new == "ddtrain":
    df_valuebench_dd = pd.read_csv(
        "../daily_dilemmas/dilemma_to_action_to_values_aggregated_train.csv"
    )
elif DATA_SPLIT_new == "ddtest":
    df_valuebench_dd = pd.read_csv(
        "../daily_dilemmas/dilemma_to_action_to_values_aggregated_test.csv"
    )
...
if NUM_VALUE_DIM != "ALL":
    if NUM_VALUE_DIM == "TEASERSET":
        smallset = [
            ...
            "Breadth of Interest",
            "Social Cynicism",
        ]  # values with 20+ questions in valuebench
...
for value_name in value_list:
    value_def_dict[value_name] = " ".join(
        [
            vd
            for vd in value_def_data[value_def_data["value"] == value_name][
                "definition"
            ].values
...
for value_name, value_qa in grouped:
    ...
    with open(
        os.path.join("valuebench_info", "value_questions_" + value_name + ".html"), "w"
    ) as f:
...
if base_model == "GPT2-SMALL":
    answer_valuebench_features_csv = (
        "answers_gpt2small"
        + "_players"
        + str(NUM_PLAYERS_GENERATE)
        + "_valuedims"
        + str(NUM_VALUE_DIM)
        + ".csv"
    )
...
if ALLOW_UNSURE_ANSWER:
    answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
        ".csv", "_unsure.csv"
    )
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_sae.csv"
...
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "NUM_PLAYERS_USE" + str(NUM_PLAYERS_USE) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "NUM_PLAYERS_START" + str(NUM_PLAYERS_START) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "SAE_STEERED_FEATURE_NUM" + str(SAE_STEERED_FEATURE_NUM) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "PERSON" + str(PERSON) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "SYSTEMATIC_PROMPT" + str(SYSTEMATIC_PROMPT) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "EXAMPLES_IN_PROMPT" + str(EXAMPLES_IN_PROMPT) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "SAE_STEERED_RANGE" + str(SAE_STEERED_RANGE) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "STEERING_COEFF" + str(STEER_COEFF) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + str(DATA_SPLIT) + ".csv"
)
answer_valuebench_features_csv = answer_valuebench_features_csv.replace(
    ".csv", "_" + "SAMPLERATE" + str(GROUP_SAMPLE_RATE) + ".csv"
)

answer_valuebench_features_csv = os.path.join(
    "useful_data", answer_valuebench_features_csv
)
answer_valuebench_features_csv_new = answer_valuebench_features_csv.replace(
    ".csv", "_NEW.csv"
)
...
if base_model == "GPT2-SMALL":
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "openai-community", "gpt2")
    )
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "google", "gemma-2b")
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "google", "gemma-2b"), padding_side="left"
    )

elif base_model == "GEMMA-2B-IT":
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "google", "gemma-2b-it")
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "google", "gemma-2b-it"), padding_side="left"
    )
    # vllm_generator = VLLMGenerator(os.path.join(LOCAL_SAE_MODEL_PATH, "google", "gemma-2b-it"))

elif base_model == "GEMMA-2B-CHN":
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "ccrains", "larson-gemma-2b-chinese-v0.1/")
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "ccrains", "larson-gemma-2b-chinese-v0.1/"),
        padding_side="left",
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "mistralai", "Mistral-7B-v0.1/")
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "mistralai", "Mistral-7B-v0.1/"),
        padding_side="left",
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "meta-llama", "Meta-Llama-3-8B")
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "meta-llama", "Meta-Llama-3-8B-Instruct")
    )  # , quantization_config=bnb_config)
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "meta-llama", "Meta-Llama-3-8B-Instruct"),
        padding_side="left",
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(
            LOCAL_SAE_MODEL_PATH, "meta-llama", "meta-llama-3-8b-instruct-helpfull"
        ),
        quantization_config=bnb_config,
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "hfl", "llama-3-chinese-8b-instruct-v3/"),
        quantization_config=bnb_config,
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join(LOCAL_SAE_MODEL_PATH, "hfl", "llama-3-chinese-8b-instruct-v3/"),
    ...
    hf_model = AutoModelForCausalLM.from_pretrained(
        os.path.join("/scratch2/nlp/plm", "Llama-3-14B-Instruct-v1"),
        quantization_config=bnb_config,
    )
    hf_tokenizer = AutoTokenizer.from_pretrained(
        os.path.join("/scratch2/nlp/plm", "Llama-3-14B-Instruct-v1"),
...
if base_model == "GPT2-SMALL":
    ...
    sae_base_dir = (
        LOCAL_SAE_MODEL_PATH
        + "/jbloom/Gemma-2b-Residual-Stream-SAEs/gemma_2b_it_blocks.12.hook_resid_post_16384/"
    )
    ...
    sae_base_dir = (
        LOCAL_SAE_MODEL_PATH
        + "/JoshEngels/Mistral-7B-Residual-Stream-SAEs/mistral_7b_layer_8"
    )
    sae = SAE.load_from_pretrained(sae_base_dir, device=device)
    # sae, original_cfg_dict, sparsity = SAE.from_pretrained(
    ...
    "LLAMA31-8B-IT",
    "LLAMA3-8B-IT-HELPFUL",
    "LLAMA3-8B-IT-FICTION",
    "LLAMA3-8B-IT-CHN",
    "GEMMA-2B-CHN",
]:
...
def judge_answer(thought_n_answer, question, rulefirst):
    ...
    if (
        thought_n_answer.startswith("yes")
        or thought_n_answer.startswith("sure")
        or pure_answer.startswith("yes")
        or pure_answer.startswith("sure")
    ):
        result_answer_auto = "yes"
    elif thought_n_answer.startswith("no") or pure_answer.startswith("no"):
        result_answer_auto = "no"
    elif (
        thought_n_answer.startswith("unsure")
        or thought_n_answer.startswith("i cannot")
        or thought_n_answer.startswith("i am unable")
        or pure_answer.startswith("unsure")
    ):
    ...
    try:
        ...
        result_answer_gpt = (
            judge_chat_completion.choices[0].message.content.strip().lower()
        )
...
def judge_answer_free(thought_n_answer, question):
    ...
    feed_content_1 = feed_content_1.format(
        value_name=",".join(value_def_dict.keys()),
        question=question,
        thought_n_answer=thought_n_answer,
    )

    tokenized = model.to_tokens([feed_content_1])
    result_answer_gpt = model.generate(
        stop_at_eos=True,
        eos_token_id=STOP_SIGNS,
        input=tokenized,
        verbose=False,
        **SAMPLING_KWARGS,
    )
    ...
    for value_name in value_def_dict.keys():
        ...
        if result_answer_gpt.startswith("yes") or result_answer_gpt.startswith("sure"):
            ...
            result_answer_gpt.startswith("unsure")
            or result_answer_gpt.startswith("i cannot")
            or result_answer_gpt.startswith("i am unable")
        ):
            result_answer_gpt = "unsure"
...
def indexing_role_prompt(whole_prompt_tokens, role_prompt_tokens):
    for i in range(len(whole_prompt_tokens)):
        ...
        if i + len(role_prompt_tokens) > len(whole_prompt_tokens):
            continue
        if torch.all(
            whole_prompt_tokens[i : i + len(role_prompt_tokens)] == role_prompt_tokens
        ):
            return i
    assert (
        False
    ), f"Role prompt not found in whole prompt: {whole_prompt_tokens}, {role_prompt_tokens}"


question_no_bio, common_sae_steered_string = generate_question_analysis(
    "", "", "", "", ALLOW_UNSURE_ANSWER, None
)
...
with torch.no_grad():
    ...
    if NUM_PLAYERS_START == -1:
        ...
        player_list = list(players.keys())[
            NUM_PLAYERS_START : NUM_PLAYERS_START + NUM_PLAYERS_USE
        ]
        ...
        for steered_dim in role_sae_counter_sorted[:SAE_STEERED_FEATURE_NUM]:
            ...
            for value_name, group in grouped:
                ...
                sample_index = random.sample(
                    range(group_len), math.ceil(group_len * GROUP_SAMPLE_RATE)
                )
                sample_index.sort()
                random.seed()
                ...
                scores = []
                question_batch_no = math.ceil(
                    len(groupagreementall) / MAX_QUESTIONS_PER_BATCH
                )
                for qbn in range(question_batch_no):
                    groupagreement = groupagreementall[
                        qbn
                        * MAX_QUESTIONS_PER_BATCH : (qbn + 1)
                        * MAX_QUESTIONS_PER_BATCH
                    ]
                    groupquestion = groupquestionall[
                        qbn
                        * MAX_QUESTIONS_PER_BATCH : (qbn + 1)
                        * MAX_QUESTIONS_PER_BATCH
                    ]
                    groupitem = groupitemall[
                        qbn
                        * MAX_QUESTIONS_PER_BATCH : (qbn + 1)
                        * MAX_QUESTIONS_PER_BATCH
                    ]
                    ...
                    for groupmember in zip(groupagreement, groupquestion, groupitem):
                        ...
                        q = groupmember[1]
                        qi = groupmember[2]
                        prompt, _ = generate_question_analysis(
                            value_name, a, q, qi, ALLOW_UNSURE_ANSWER, trait
                        )
                    ...
                    def steering_hook(resid_pre, hook):
                        ...
                        if STEERING_ON:
                            if STEER_LOC == "out":
                                for batch_no, startend in enumerate(startend_positions):
                                    start, end = startend
                                    resid_pre[batch_no, start:end, :] += (
                                        STEER_COEFF * steering_vector
                                    )
                                # resid_pre[:,:,:] = STEER_COEFF * torch.rand_like(resid_pre)
                            elif STEER_LOC == "in":
                                sv_feature_acts = sae.encode(resid_pre)
                                # sv_feature_acts[:, :position, steered_dim] *= 0#STEER_COEFF
                                sv_feature_acts[:, :, :] = torch.zeros_like(
                                    sv_feature_acts
                                )
                                # sv_feature_acts[:, :position, :] = 1000 * STEER_COEFF * torch.rand_like(sv_feature_acts[:, :position, :])
                                resid_pre[:, :, :] = sae.decode(sv_feature_acts)
                    ...
                    def hooked_generate(
                        prompt_batch, fwd_hooks=[], seed=None, **kwargs
                    ):
                        if seed is not None:
                            torch.manual_seed(seed)
                        with model.hooks(fwd_hooks=fwd_hooks):
                            ...
                            startend_positions.clear()
                            for tokensquence in tokenized:
                                index_start = indexing_role_prompt(
                                    tokensquence, common_sae_steered_string_tokens
                                )
                                index_end = index_start + len(
                                    common_sae_steered_string_tokens
                                )
                                startend_positions.append((index_start, index_end))
                    ...
                    def run_generate(prompts):
                        ...
                        res_str = [
                            output_result[0]["generated_text"]
                            for output_result in outputs_result
                        ]
                        ...
                        for pro, rs in zip(prompts, [rs for rs in res_str]):
                            ...
                            question_count += 1
                        return [
                            judge_answer(rs.split(pr)[-1], gm, JUDGE_ANSWER_RULE_FIRST)
                            for rs, gm, pr in zip(res_str, groupquestion, prompts)
                        ]
                ...
                gen_answers_all0 = [
                    ga * sa for ga, sa in zip(groupagreementall, scores0[value_name])
                ]
                changed_scores = []
                changed_scores_count = Counter()
                for el, ga, gaa, gaa0, sa, sa0 in zip(
                    range(len(scores)),
                    groupagreementall,
                    gen_answers_all,
                    gen_answers_all0,
            ...
            if not head_added:
                pd.DataFrame(columns=pd_row.keys()).to_csv(
                    answer_valuebench_features_csv, index=False
                )
                head_added = True
            pd_row.to_csv(
                answer_valuebench_features_csv, mode="a", index=False, header=False
            )
